...
def spark_tools(data, keep_range=(0, 5000),
    ...
    if enable_iv_filter or enable_single_filter:
        ...
        if enable_iv_filter is True:  # iv筛选变量
            log("开始进行iv筛选")  # todo:还需要优化性能
            ivs = dict()
            dist = rdd.flatMap(lambda x: attr_cnt2(x, attr_list, response=response)).reduceByKey(add).collect()
            df = pd.DataFrame([(attr, target, val, count) for ((attr, val, target), count) in dist], columns=['attr', 'code', 'value', 'number']).set_index(
                ['attr', 'code', 'value'])
            good_total = df.xs(attr_list[0]).xs(good).sum()
            bad_total = df.xs(attr_list[0]).xs(bad).sum()
            for _ in attr_list:
                tmp_ = df.xs(_)
                df_tmp = pd.concat([tmp_.xs(good), tmp_.xs(bad)], axis=1)
...
def write_to_local(result, filename, local_path=None, drop_tmp=True):
    ...
    log("合并列名和数据")
    df = pd.read_csv(local_tmp_path, names=header)
